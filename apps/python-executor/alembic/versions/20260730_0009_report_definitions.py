...
def upgrade() -> None:
    from app.models import ReportDefinition  # noqa: WPS433

    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "report_definitions" not in inspector.get_table_names():
        ReportDefinition.__table__.create(bind=bind)
...
def downgrade() -> None:
    ...
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "report_definitions" in inspector.get_table_names():
        ReportDefinition.__table__.drop(bind=bind)
